...
/**
 * Lightweight cached check that migration 016 tables exist.
 * Does not run DDL — schema is created only via php migrate.php.
 */
function api_credentials_schema_ready(): bool
{
    static $ready = null;
    ...
    try {
        $ready = database_table_exists('api_credentials')
            && database_table_exists('api_request_logs')
            && database_table_exists('api_rate_limits');
...
function api_credentials_feature_ready(): bool
{
    return api_credentials_schema_ready() && api_credentials_crypto_ready();
}

function require_api_credentials_schema(): void
{
    if (!api_credentials_schema_ready()) {
        throw new RuntimeException('API credentials schema is not configured.');
    }
}
...
function create_or_replace_api_credential(
    ...
    require_api_credentials_schema();
